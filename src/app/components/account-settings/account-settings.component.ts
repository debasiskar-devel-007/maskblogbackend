...
export class AccountSettingsComponent implements OnInit {
  ...
  constructor(public meta: MetaService, public fb: FormBuilder,
    ...
    this.meta.setTitle('Virus Barrier Medical Face Mask Blog backend | My Account');
    ...
    this.meta.setTag('og:title', 'Virus Barrier Medical Face Mask Blog backend | My Account');
    this.meta.setTag('twitter:title', 'Virus Barrier Medical Face Mask Blog backend | My Account');
  ...
  changePasswordFormSubmit() {
    ...
    if (this.changePasswordFormGroup.valid) {
      ...
      this.apiService.addUpdateService(data, endpoint).subscribe((res:any) => {
        if(res.Status=='true'){
  ...
  getdata() {
    let data: any = {
      endpoint: 'datalist',
      source: 'data_user_view',
      condition: {
        "_id_object": this.cookies_id
      }
    }
    this.apiService.getDatalistforapi1(data).subscribe((res: any) => {
